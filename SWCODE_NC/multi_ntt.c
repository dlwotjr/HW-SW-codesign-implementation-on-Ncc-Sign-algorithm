...
void poly_mul_NTT_ayz_verify(poly *res, poly *a, poly *b, poly *c, poly *d)
{
	...
	for (i = 0; i < 2 * N - 1; i++)
	{
		E_q1[i] = unsigned2signed(E_q1[i], Q_NTT_MULTI_Q1);
		E_q2[i] = unsigned2signed(E_q2[i], Q_NTT_MULTI_Q2);
	}
	for (i = N; i < 2 * N - 1; i++) {
	...
	for (i = 0; i < N; i++) {
		res_q1.coeffs[i] = E_q1[i];
		res_q2.coeffs[i] = E_q2[i];
	}
