...
int crypto_sign_keypair(uint8_t *pk, uint8_t *sk) {
	...
	poly ms1, mt,mt_q1,mt_q2, test,test2;
	int64_t result[N] = {0};
						  ...
						  const uint8_t *sk)
{
	...
	poly cp;
	keccak_state state;
	poly ms1, ms2, mt0, mz, mw1, mcp, mh, mw1_q1, mw1_q2;
...
rej:
	...
	poly_mul_NTT_multi_q1_pre(&mw1_q1, hw_matq1, &z);
	poly_mul_NTT_multi_q2_pre(&mw1_q2, hw_matq2, &z);
	ntt_crt(&w1,&mw1_q1,&mw1_q2);
	...
	poly_mul_NTT_multi_q1_pre(&mz,hw_cp,&s1);

	poly_add(&z, &mz, &y);
	if (poly_chknorm(&z, GAMMA1 - BETA))
		goto rej;

	/*poly_mul_NTT_ct(&mh, &cp, &s2);*/
	poly_mul_NTT_multi_q1_pre(&mh, hw_cp, &s2);
	poly_sub(&w0, &w0, &mh);
	...
	//poly_mul_NTT_ct(&mh, &cp, &t0);
	poly_mul_NTT_multi_q1_pre(&mh, hw_cp, &t0);

	if (poly_chknorm(&mh, GAMMA2))
		goto rej;
					   ...
					   const uint8_t *pk)
{
	...
	keccak_state state;
	poly mz, mw1, mcp, mt1, mt1_temp, mw1_q1,  mw1_q2, mt1_q1, mt1_q2;
